...
define(['N/record', 'N/search'],
	...
	function(record, search) {
		...
		function is_stf_setTaxFields(context){
			...
			var splitGSTPST = rec.getValue({
				fieldId: 'custbody_scg_split_gst_pst'
			})
			log.debug('Split GST/PST', splitGSTPST);
			...
			for (var i = 0; i < itemCount; i++){
				...
				if (nexusCountry == 'CA' && (shipState == MANITOBA || shipState == QUEBEC || shipState == BRITISH_COLUMBIA || shipState == SASKATCHEWAN)){
					...
					//If Subsidiary = insightsoftware, LLC and Split GST/PST is not checked, Tax Rate 1 = 0, remaining to Tax Rate 2, else normal logic
					if (subsidiary == INSIGHTSOFTWARE_LLC && splitGSTPST == false){
